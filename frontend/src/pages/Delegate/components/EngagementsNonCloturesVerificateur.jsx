...
import { getNonClosedByVerificateur } from "../../../services/delegateService";

function EngagementsNonCloturesVerificateur({ user }) {
  ...
  const loadEngagements = async () => {
    setLoading(true);
    try {
      const data = await getNonClosedByVerificateur(
        user?.immatricule,
        user?.annee
      );
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-[#0B1F44]">Engagements Non Vérifiés</h2>
          <p className="text-gray-500 text-sm">Liste des engagements réceptionnés par le vérificateur non encore clôturés</p>
        </div>
      ...
      {engagements.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          <p className="text-gray-500">Aucun engagement en attente de clôture par le vérificateur.</p>
        </div>
      ) : (
...
export default EngagementsNonCloturesVerificateur;