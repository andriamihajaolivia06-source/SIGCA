...
import { getClosedEngagements } from "../../../services/secretaryService";

function ListeClotures({ user }) {
  ...
  const loadEngagements = async () => {
    setLoading(true);
    try {
      const data = await getClosedEngagements(
        user?.immatricule,
        user?.annee
      );
      setEngagements(data.results || []);
    } catch (error) {
      console.error("Erreur chargement engagements clôturés:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredEngagements = engagements.filter(item => {
    ...
    return (
      ...
      (item.loginCloture && item.loginCloture.toLowerCase().includes(search))
    );
  });
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Chargement des engagements clôturés...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-[#0B1F44]">Liste des Clôtures</h2>
          <p className="text-gray-500 text-sm">Liste des engagements déjà clôturés et envoyés au vérificateur</p>
        </div>
      ...
      {engagements.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          <p className="text-gray-500">Aucun engagement clôturé pour le moment.</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 flex items-center justify-between">
            <h3 className="text-lg font-semibold text-[#0B1F44]">
              Engagements clôturés ({filteredEngagements.length})
            </h3>
